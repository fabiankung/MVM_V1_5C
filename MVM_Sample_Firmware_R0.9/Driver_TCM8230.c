...
///                       3. Pin PB2 (Output) = Enable pin to power switch (active low).
///                       4. Pin PA3-PA5, PA9-PA13 (Inputs) = PIODC0 to PIODC7 (Data0-7 of camera)
///                       5. Pin PA22 (Input) = PIODCCLK (Dclk)
///                       6. Pin PA14 (Input) = PIODCEN1 (VSysnc)
///                       7. Pin PA21 (Input) = PIODCEN2 (HSync)
///
...
/// on-chip peripheral PCK0.  At present the output of PCK2 can be set to 6 MHz, 12MHz or
/// 16.67 MHz, corresponding to camera frame rate of 7.5 fps, 15 fps and 20.83 fps. At present
/// depends on situation, sometimes the camera cannot initialize properly at a particular
/// frame rate, so the user has to try a few combination.
/// For default we are using 20.83 fps at a resolution of QQVGA.
...
void Proce_TCM8230_Driver(TASK_ATTRIBUTE *ptrTask)
{
	...
	if (ptrTask->nTimer == 0)
	{
		switch (ptrTask->nState)
		{
			...
			case 1: // State 1 - 
				PIOB->PIO_ODSR &= ~PIO_ODSR_P2;											// Turn on camera power switch.  
			...
			case 3: // State 3 - Disable camera reset, delay.
				...
				//OSSetTaskContext(ptrTask, 5, 200*__NUM_SYSTEMTICK_MSEC);		// Next state = 5, timer = 500 msec.																	
			break;
				...
				{	
					...
					{				
						...
						{												// processes.  So we should update gunImgAtt2[].
							...
							//PIN_FLAG2_SET;		
							//gnValidFrameBuffer = 1;					    // Indicating odd frame number being updated.						
						}
						...
						{	
							...
							//PIN_FLAG2_CLEAR;						
							//gnValidFrameBuffer = 0;						// Indicating even frame number being updated.
						}
			...
			case 11: // State 11 - End, do some tidy-up chores, update frame counter and valid frame buffer flag.
				...
				// --- Test routines ---
				// Sample PD21 state.
				if ((PIOD->PIO_PDSR) & 0x00200000)
				{												// If pushbutton switch is not closed, high.
					if (gnCameraLED == 0)
					{
						gnCameraLED = 1;						// Lights up camera LED at lowest intensity if it is off.
					}
					OSSetTaskContext(ptrTask, 8, 1);			// Next state = 8, timer = 1.
				}
				else
				{
																// Pushbutton switch is closed, low.
			...
			case 13: // State 13 - Power down sequence, turn off camera power switch.
				PIOB->PIO_ODSR |= PIO_ODSR_P2;					// Turn off camera power switch. 
