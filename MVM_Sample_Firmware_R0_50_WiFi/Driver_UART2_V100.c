...
//
//  (c) Copyright 2018-2019, Fabian Kung Wai Lee, Selangor, MALAYSIA
//  All Rights Reserved  
//   
//////////////////////////////////////////////////////////////////////////////////////////////
//
	...
	if (ptrTask->nTimer == 0)
	{
		switch (ptrTask->nState)
		{
			case 0: // State 0 - UART2 Initialization.
				...
				// Setup pin PD25 as input. General purpose input with internal pull-up.
				...
				if (_UART_BAUDRATE_kBPS == 460.8)
				{
					UART2->UART_BRGR = 20;						// Approximate 468.75 kbps, -1.73%
				}
                else if (_UART_BAUDRATE_kBPS == 345.6)			
                {
	                UART2->UART_BRGR = 27;						// Approximate 347.22 kbps, -0.47%
                }				
                else if (_UART_BAUDRATE_kBPS == 230.4)
				{
					UART2->UART_BRGR = 41;						// Approximate 228.66 kbps, 0.76%
				}
				else if (_UART_BAUDRATE_kBPS == 115.2)
				{
					UART2->UART_BRGR = 81;						// Approximate 115.74 kbps, -0.46%
				}
				else
				{
...
/* This is shifted to Systick interrupt service routine, see SysTick_Handler(void). 
				...
				// Note that the receive FIFO buffer is only 2-level deep in ATSAMS70 Cortex-M7 micro-controllers.
                ...
                {															// Make sure no hardware overflow and 
																			// and framing error (when STOP bit is detected as '0').
