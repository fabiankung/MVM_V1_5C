	...
	if (ptrTask->nTimer == 0)
	{
		switch (ptrTask->nState)
		{
			...
			case 2: // State 2 - Settings of PCK2 (Programmable Clock 2, pin PB3) to generate the clock signal for the camera.
				...
				*/								
				OSSetTaskContext(ptrTask, 3, 500*__NUM_SYSTEMTICK_MSEC);		// Next state = 3, timer = 100 msec.
			break;			
			...
			//OSSetTaskContext(ptrTask, 3, 20);
				if (gI2CStat.bI2CBusy == 0)				// Make sure I2C module is not being used.
				{
					...
					gbytI2CTXbuf[0] = 0x68;				// Data, normal mode.
			...
			case 10: // State 10 - Wait until DMA transfer of 1 line of pixel data is complete.										
				...
				{	
					...
					{				
						...
						// [Byte1][Byte0].
						unTemp = __REV16(gn16Pixel[ncolindex]);	// Swap the position of lower and upper 8 bits!
						...
						// Temporary disable Sobel kernel computation to increase user processing bandwidth
						/*
