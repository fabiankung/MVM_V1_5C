...
//                	  GCC C-Compiler 
//					  ARM CMSIS 5.0.1
// Micro-controller	: Atmel SAMS70J19 or SAMS70J21 ARM Cortex-M4 families.
...
///                   4. Setup the SysTick system timer.
...
{	
	...
	PIOB->PIO_OWER = 0xFFFFFFFE;	// Enable the write operation to PIO_ODSR register.
									...
									// PD25 is RX of UART0, so it should remain as input.
	...
	// Note: 16 Oct 2015, the following is not needed, by default SysTick is being triggered with processor clock HCLK.
	// The SysTick module is triggered from the output of the HCLK divided by 8.  
	// the timeout for SysTick = [SysTick Value] x 8 x (1/fCore).
	// For fCore = 300 MHz, SysTick Value = 6250
	// This will give SysTick time out around 166.67 usec.
